...
int merge_count=30;
int cold_count=0;
static int lru_bound=3;//如果一个大页的访问频率超过该值，则表示该页为热页
static int merge_sleep_millisecs=1500;
static int merge_sleep_millisecs1=6000;
static int small_hot_bound=128;//如果一个大页有128个小页被访问了，则将该大页的hot计数值加1，大页的hot计数值超过lru_bound则该大页为热页
 ...
 * NULL otherwise.
 */
static struct page *stable_tree_search(struct page *page, struct rmap_item *rmap_item)
{
	int nid;
	struct rb_root *root;
	struct rb_node **new;
...
again:
	...
	while (*new) {
		...
		//ret = memcmp_checksum(rmap_item->oldchecksum, stable_node->oldchecksum);
...
static struct stable_node *stable_tree_insert(struct page *kpage)
{
	...
	stable_node->oldchecksum= calc_checksum(kpage);
...
next_mm:
		...
		while (merge_scan.address < vma->vm_end) {
			...
			*page = follow_page(vma, merge_scan.address, FOLL_GET);
			...
			if (PageAnon(*page) ||
				...
				flush_dcache_page(*page);
				rmap_item = my_get_next_rmap_item(slot,
	...
	//remove_trailing_rmap_items(slot, merge_scan.rmap_list);
...
{
	struct rmap_item *rmap_item;
	struct page *uninitialized_var(page), *page1=NULL;
	unsigned int checksum;
...
static void merge_do_scan(unsigned int scan_npages)
{
	...
	while (seqnr_begin==merge_scan.seqnr&& likely(!freezing(current))) {
		...
		if(PageTransCompound(page)&&PageHead(page)){
			...
			if(rmap_item->head_item->rank<1600){
			...
			switch(page_referenced(page,1,pc,&vm_flags)){
				case 0:
